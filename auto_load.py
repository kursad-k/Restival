try:
    ...
    bpy = None  # type: ignore[assignment]  # running outside Blender (unit tests)
import typing
import inspect
import pkgutil
import importlib
...
blender_version = bpy.app.version if bpy is not None else (0, 0, 0)
...
def iter_submodule_names(path, root=""):
    for _, module_name, is_package in pkgutil.iter_modules([str(path)]):
        # Skip test/spec directories — they are not Blender addon submodules
        if root == "" and module_name in _EXCLUDED_PACKAGES:
            continue
        if is_package:
